...
if($#ARGV != 1){
  print "Must give two pdb arguments\n";
  exit;
}
...
print "Comparing $pdb1 and $pdb2\n";
my @tokens = split('/',$pdb2);
my $name_length = length($tokens[-1]);
my $pdb2_substr = substr($tokens[-1], 0, $name_length - 4 );

system("grep \" CA \" $pdb1 > pdb1");
system("grep \" CA \" $pdb2 > pdb2");
...
while ( <PDB2> ) {
	...
	#set up mut outputs
	my $out = $out . $seq1[$i] . $res2[$i] . $seq2[$i]. " ";
