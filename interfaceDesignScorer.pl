...
my $count = 0;
if(! $verbose){
	print "desc\tenergy\trmsd\tddg\tsasa\tunsats\thbond energy\tpackstat\tscvalue\t(dg/sasa)\t(sasa/unsat)\n";
}
		...
		if($count==0){
			...
			if( (!$scoreCut or $energy <= $scoreCut) and 
					...
					if($verbose){
						...
						#$desc =~ s/.*_//g;
						print "$desc\t$energy\t$rmsd\t$ddg\t" . sprintf("%.0f", $sasa) . "\t$unsats\t$hbondEnergy\t$packstat\t$scValue\t" 
							. sprintf("%.3f", $ddgSasa) . "\t" . sprintf("%.2f", $sasaUnsat) . "\n";
					}
			}
		}
		$count++;
